...
export class AddPassengersComponent implements OnInit {
  ...
  addMorePassengers() {
    this.route.paramMap.subscribe(params =>{
      var busNumber=params.get('busNumber');

      let response=this.userService.getBusByNumber(busNumber).subscribe((data:any)=>this.bus=data)
  })
    
    let a=this.passengerArray.length
    if(a<this.bus.availableSeats){
    ...
    // console.log(a +"   "+ this.bus.availableSeats)
